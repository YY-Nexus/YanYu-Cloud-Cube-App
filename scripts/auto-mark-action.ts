...
function mark(item: any) {
  ...
  if (item.hasPackage === false || !item.nextVersion) return { ...item, recommendedAction: "ARCHIVE" };
  // 体积小于10MB且只有pages
  ...
  if (parseFloat(item.nextVersion.replace(/[^\d.]/g, "")) < 14) return { ...item, recommendedAction: "MERGE" };
  // 其它情况
